...
# PURPOSE: Pull in all OC data for time series figure for Jayson Smith
...
CRANE_data_prep(SELECT = "OC_sites",
...
dat_event.OC <- data.table(dat_event)
...
dat_kelp <- data.table(dat_kelp)
  ...
  #Site names
  sites_keep <- unique(dat_event.OC$Site)
  
  dat_fish_t.OC <- dat_fish_t[Site %in% sites_keep,]
  dat_macroinvert.OC <- dat_macroinvert[Site %in% sites_keep,]
  dat_kelp.OC <- dat_kelp[Site %in% sites_keep,]
  
  ########################
  ##Remove TBF 2016 Project (ASK WHAT THIS IS AND IF I SHOULD LEAVE IT IN)
  ########################
  dat_event.OC <- dat_event[Project != "TBF2016",]
  dat_fish_t.OC <- dat_fish_t.OC[Project != "TBF2016",]
  dat_macroinvert.OC <- dat_macroinvert.OC[Project != "TBF2016",]
  dat_kelp.OC <- dat_kelp.OC[Project != "TBF2016",]

  ########################
  ##Take average values across all years of sampling (avg density and avg biomass of each species at site)
  ########################
  
  dat_fish_site_averages_OC <- dat_fish_t.OC[,.(mean_density_m2=mean(density_m2), mean_wt_density_g_m2=mean(wt_density_g_m2)),
                                         .(Species, Region, AR_Complex, Site, DepthZone)]
  dat_macroinvert_site_averages_OC <- dat_macroinvert.OC[,.(mean_density_m2=mean(Abundance/area.m2)),
                                                     .(BenthicReefSpecies, SpeciesGroupF, Region, AR_Complex, Site, DepthZone)]
  dat_kelp_site_averages_OC <- dat_kelp.OC[,.(mean_density_m2=mean(Abundance/area.m2)),
                                       .(BenthicReefSpecies, SpeciesGroupF, Region, AR_Complex, Site, DepthZone)]
  
  #Which regions are retained?
  unique(dat_event.OC$Region)
  
  #What years do we have data from?
  unique(dat_event.OC$SampleYear) #2008, 2004, 2007, 2009, 2019, 2020, 2021, 2022, 2023
  
  ########################
  ##Save average output
  ########################
  saveRDS(dat_event.OC, file.path("data","processed_crane", "dat_event_OC.rds"))
  saveRDS(dat_fish_site_averages_OC, file.path("data","processed_crane", "dat_fish_site_averages_OC.rds"))
  saveRDS(dat_macroinvert_site_averages_OC, file.path("data","processed_crane", "dat_macroinvert_site_averages_OC.rds"))
  saveRDS(dat_kelp_site_averages_OC, file.path("data","processed_crane", "dat_kelp_site_averages_OC.rds"))
